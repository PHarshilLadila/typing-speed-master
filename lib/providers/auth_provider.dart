...
class AuthProvider with ChangeNotifier {
  ...
  Future<void> signInWithGoogle() async {
    try {
      _isLoading = true;
      _error = null;
      _isSignOut = false; // Reset sign out flag when signing in
      notifyListeners();
  ...
  Future<void> _fetchUserProfile(String userId) async {
    try {
      // Don't fetch profile if we're in sign out process
  ...
  Future<void> _createUserProfile(String userId) async {
    try {
      // Don't create profile if we're in sign out process
      if (_isSignOut) return;
  ...
  Future<void> signOut() async {
    try {
      _isLoading = true;
      _isSignOut = true; // Set sign out flag to true
      notifyListeners();

      // Add a small delay to ensure UI updates before sign out
      ...
      _isLoading = false;
      _isSignOut = false; // Reset sign out flag
      notifyListeners();
    }
  }

  // Method to check if sign out is in progress
  bool get isSigningOut => _isSignOut || _isLoading;

  // Method to manually set sign out state (useful for UI)
  void setSignOutState(bool value) {
    _isSignOut = value;
    notifyListeners();
  }

  // Cancel any ongoing operations when sign out is detected
  ...
  // Reset the provider state (useful for testing or complete reset)
  void reset() {
    _user = null;
    _isLoading = false;
    _error = null;
